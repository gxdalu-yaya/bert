...
class IcsProcessor(DataProcessor):
  """Processor for the XNLI data set."""
  ...
  def get_train_examples(self, data_file):
    ...
    for line in open(data_file):
      ...
      guid = "train-%d" % (i)
      if i % 10000 == 0:
        print("Reading example %d " % (i))
      i += 1
      text_a = tokenization.convert_to_unicode(datas[0])
      text_b = tokenization.convert_to_unicode(datas[1])
      label = tokenization.convert_to_unicode(datas[2])
      if label == tokenization.convert_to_unicode("contradictory"):
        label = tokenization.convert_to_unicode("contradiction")
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
  ...
  def get_dev_examples(self, data_dir):
    ...
    examples = []
    i = 1
    for line in open(os.path.join(data_dir, "dev.txt")):
      datas = line.strip().split("\t")
      guid = "dev-%d" % (i)
      i += 1
      text_a = tokenization.convert_to_unicode(datas[0])
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      output_spec = tf.estimator.EstimatorSpec(
          mode=mode,
          predictions={'class': predicted_classes, 'prob': tf.nn.softmax(logits)}, export_outputs=export_outputs,
          #scaffold_fn=scaffold_fn
          )
    else:
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    #                             FLAGS.max_seq_length, tokenizer, train_tfrecord_files)
    ...
    def _f1_bigger(best_eval_result, current_eval_result):
      ...
      default_key = "eval_precision".decode("utf-8")
      ...
      best_precision_score = best_eval_result["eval_precision".decode("utf-8")] 
      best_recall_score = best_eval_result["eval_recall".decode("utf-8")]
      best_acc_score = best_eval_result["eval_accuracy".decode("utf-8")]
      best_f1_score = 2*best_precision_score*best_recall_score/(best_precision_score+best_recall_score)

      current_precision_score = current_eval_result["eval_precision".decode("utf-8")] 
      current_recall_score = current_eval_result["eval_recall".decode("utf-8")]
      current_acc_score = current_eval_result["eval_accuracy".decode("utf-8")]
      current_f1_score = 2*current_precision_score*current_recall_score/(current_precision_score+current_recall_score)

      print("best_eval_result")
      print(best_eval_result)
      print("current_eval_result")
      print(current_eval_result)

      print("best_acc_score: {}".format(best_acc_score))
      print("best_precision_score: {}".format(best_precision_score))
      print("best_recall_score: {}".format(best_recall_score))
      print("best_f1_score: {}".format(best_f1_score))
      print("current_f1_score")
      print(current_f1_score)
      return best_f1_score < current_f1_score
      #return best_acc_score < current_acc_score
    ...
    train_log = tf.estimator.train_and_evaluate(estimator, train_spec, eval_spec)
    print("train_log")
    print(train_log)
